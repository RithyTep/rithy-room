...
export function YouTubePlayer() {
    ...
    playerRef.current = new window.YT.Player('youtube-player', {
      ...
      events: {
        onReady: (event) => {
          ...
          setVideoTitle(player.getVideoData().title || 'YouTube Video');
  ...
  useEffect(() => {
    if (!musicState || !playerRef.current || musicState.updatedBy === currentMemberId) return;

    const player = playerRef.current;
    const timeDiff = Math.abs(player.getCurrentTime() - musicState.currentTime);
    ...
    if (musicState.playing && !isPlaying) {
      player.playVideo();
    } else if (!musicState.playing && isPlaying) {
      player.pauseVideo();
    }
  }, [musicState, currentMemberId, isPlaying]);
  ...
  const handlePlayPause = useCallback(() => {
    ...
    if (isPlaying) {
      player.pauseVideo();
      syncMusic(musicState.url, false, currentTime);
    } else {
      player.playVideo();
      syncMusic(musicState.url, true, currentTime);
    }
    ...
    >
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-[#555] font-mono w-20">
